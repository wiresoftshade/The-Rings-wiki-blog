...
- `S_SpaceBodyProperties` - данные о космическом теле. Класс, Имя, флаг `bIsTarget` (цель игрока). Применяется в генерации этих параметров для каждого тела созданного генератором. 
...
- Ring
    - Не передает нужную полноту списка данных: текстуры с данными
- Cluster
    - Трекам и кластерам Не передает нужную полноту списка данных: данные о разметке текстур
