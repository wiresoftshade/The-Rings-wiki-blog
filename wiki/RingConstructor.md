...
- `S_RingProperty` - Содержит данные настройки материала `RingFar`. Управляющий флаг `SpawnableAsteroids` для дебагинга. Данные размерностей и состава. Набор старый, нужно удалить лишнее, вставить актуальное.
...
3. Созданы структуры: `S_RingProperty`, `S_ClusterProperty`, `S_AsteroidProperty`. 
    ...
    3.1 Решено делать 2 вида данных: Первые исп в генераторе мира. Вторые являются разметкой и размерностями. Передаются в каждый Ring, Cluster, Asteroid. 
...
10. Кластеры занимаются лишней работой. Они расчитывают штуки, которые могут быть посчитаны в Ring. Мало того кластеры одинаковые везде и могут быть расчитаны в GameInstance. Связана с проблемой (3) см Генератор мира.